...
double TrapezoidalRule(double a, double b, int& count)
{
	...
	do {
		...
		h /= 2;
		count++;
	} while (abs(result - result_prev) > 3 * 1e-4);
	return result;
}
...
void main()
{
	...
	a = -1.0, b = 1.0, result = 0;
	double c = -1.0, d = 1.0;
	cout << " Intervals: a = 1.0 , b = -1.0 | c = 1.0 d = 1.0 \n";
	iter = 0;
